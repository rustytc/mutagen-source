...
func _process(delta):
	...
	if get_viewport().gui_get_focus_owner() is Button:
		if (Input.is_action_just_pressed("ui_accept") or (Input.is_action_just_pressed("Left Click") and get_viewport().gui_get_focus_owner() == Control and get_viewport().gui_get_focus_owner().is_hovered()) and get_viewport().gui_get_focus_owner().visible == true):
			_play_accept()
